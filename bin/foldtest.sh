#!/bin/bash
#long-range comradesFold/ comradesFold2

#create input hyb and fasta files for long-range comradesFold

#example code
#./comradesFold2 -i ZAFR.hyb -f ZAFR.fasta -a 3600 -b 5500 -g 500
#input for folding between 3601-4100 and 5501-5600
#separate script for just creating input at /homes/s2003426/bin/lr_input.sh
...
#pipe output files to comradesFold pipeline
comradesMakeConstraints -i $OUT_FILE -f $OUT_FASTA -b 1 -e $LEN
#qsub comradesFold -c ${OUT_FILE/.hyb/.1-$(($LEN))_folding_constraints.txt} -i $OUT_FASTA -s 1 &
comradesFold -c ${OUT_FILE/.hyb/.1-$(($LEN))_folding_constraints.txt} -i $OUT_FASTA
